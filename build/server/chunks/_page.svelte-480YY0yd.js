import { R as head, U as attr, S as escape_html, T as ensure_array_like, V as attr_class, W as stringify, K as derived } from './dev-BjcCn9Qu.js';
...
function _page($$renderer, $$props) {
	$$renderer.component(($$renderer) => {
		...
		let alumnoQ = data.alumnoQ;
		let cursoQ = data.cursoQ;
		...
		$$renderer.push(`<div class="space-y-4"><div class="flex items-center justify-between"><h2 class="text-lg font-bold text-gray-900">Amonestaciones</h2> <a href="/preceptor/amonestaciones/nueva" class="btn-primary text-sm">+ Nueva</a></div> <div class="bg-white rounded-xl border border-gray-200 p-3 space-y-2"><div class="flex gap-2"><input type="search"${attr("value", alumnoQ)} placeholder="🔍 Buscar alumno..." class="form-input flex-1 text-sm"/> <input type="search"${attr("value", cursoQ)} placeholder="📚 Curso..." class="form-input w-28 text-sm"/></div> <div class="flex gap-2"><button class="flex-1 bg-indigo-600 text-white text-xs font-medium py-1.5 rounded-lg hover:bg-indigo-700 transition-colors">Buscar</button> `);
		if (hayFiltros()) {
			$$renderer.push("<!--[0-->");
			$$renderer.push(`<button class="text-xs text-gray-400 hover:text-gray-700 px-2">Limpiar</button>`);
		...
		for (let $$index = 0, $$length = each_array.length; $$index < $$length; $$index++) {
			...
			$$renderer.push(`<a${attr("href", buildUrl({
				gravedad: val,
				page: "1"
			}))}${attr_class(`text-xs px-3 py-1.5 rounded-full border whitespace-nowrap transition-colors ${stringify(data.gravedad === (val || null) ? "bg-indigo-600 border-indigo-600 text-white" : "border-gray-300 text-gray-600 hover:border-indigo-300")}`)}>${escape_html(lbl)}</a>`);
		}
		$$renderer.push(`<!--]--></div> `);
		if (data.lista.length === 0) {
			...
			if (data.page > 1) {
				$$renderer.push("<!--[0-->");
				$$renderer.push(`<a${attr("href", buildUrl({ page: String(data.page - 1) }))} class="text-sm text-indigo-600 hover:underline">← Anterior</a>`);
			} else $$renderer.push("<!--[-1-->");
			$$renderer.push(`<!--]--> <span class="text-sm text-gray-500">Página ${escape_html(data.page)}</span> `);
			if (data.lista.length === 20) {
				$$renderer.push("<!--[0-->");
				$$renderer.push(`<a${attr("href", buildUrl({ page: String(data.page + 1) }))} class="text-sm text-indigo-600 hover:underline">Siguiente →</a>`);
			} else $$renderer.push("<!--[-1-->");
			$$renderer.push(`<!--]--></div>`);
		}
		$$renderer.push(`<!--]--></div>`);
...
//# sourceMappingURL=_page.svelte-480YY0yd.js.map
