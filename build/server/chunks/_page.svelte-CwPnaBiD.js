...
//#region src/routes/agrupamientos/nueva/+page.svelte
function _page($$renderer, $$props) {
	$$renderer.component(($$renderer) => {
		let { data, form } = $$props;
		(/* @__PURE__ */ new Date()).toISOString().split("T")[0];
		head("2q686e", $$renderer, ($$renderer) => {
			$$renderer.title(($$renderer) => {
				$$renderer.push(`<title>Nueva sesión de agrupamiento — Legajo</title>`);
			});
		});
		$$renderer.push(`<div class="space-y-4"><div class="flex items-center gap-2"><a href="/agrupamientos" class="text-indigo-600 text-sm hover:underline">← Agrupamientos</a> <h2 class="text-lg font-bold text-gray-900">Nueva sesión</h2></div> `);
		if (form?.error) {
			$$renderer.push("<!--[0-->");
...
//# sourceMappingURL=_page.svelte-CwPnaBiD.js.map
