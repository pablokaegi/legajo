...
function _page($$renderer, $$props) {
	$$renderer.component(($$renderer) => {
		...
		$$renderer.push("<!--[-1-->");
		$$renderer.push(`<!--]-->`);
		$$renderer.push(`<!--]--></div> <div class="card space-y-2"><div class="flex items-center justify-between"><p class="text-sm font-semibold text-gray-800">Votación</p> <span class="text-sm text-gray-500">${escape_html(votos().length)} de ${escape_html(roster().length)}</span></div> <div class="h-2 bg-gray-100 rounded-full overflow-hidden"><div class="h-full bg-indigo-600 transition-all"${attr_style(`width: ${stringify(progreso())}%`)}></div></div> <a${attr("href", `/institucional/agrupamientos/${stringify(sesion().id)}/votar`)}${attr_class(`btn-primary w-full text-sm text-center block ${stringify(cerrada() ? "opacity-50 pointer-events-none" : "")}`)}>🗳️ Abrir votación (kiosko)</a> <p class="text-xs text-gray-500">Abrí esto en la compu/tablet del aula y los alumnos pasan de a uno a votar.</p></div> <div class="card space-y-2"><p class="text-sm font-semibold text-gray-800">📱 Link para votar desde el celular <span class="text-gray-400 font-normal">(opcional)</span></p> `);
		if (linkPublico()) {
			$$renderer.push("<!--[0-->");
...
//# sourceMappingURL=_page.svelte-BU3M-ZR_.js.map
