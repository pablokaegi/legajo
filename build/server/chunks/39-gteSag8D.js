import { d as db, o as observaciones, a as amonestaciones, f as faltasAlumnos, p as actasAlumnos, x as faltas, m as actas, n as actasAsistentes } from './db-BwfbdrtT.js';
import { e as esDirectivoOAdmin } from './authz-CM_w6bbu.js';
import { v as obtenerHistorialAgrupamientosDeAlumno } from './agrupamientos-Bk5OlPpq.js';
...
*/
async function obtenerLegajoAlumno(alumnoMoodleId, acceso) {
	...
	if (actaIds.length > 0) {
		...
		if (!acceso.verTodas) {
			const idsComoAsistente = db.select({ id: actasAsistentes.actaId }).from(actasAsistentes).where(eq(actasAsistentes.usuarioId, acceso.usuarioId));
			conds.push(or(eq(actas.createdBy, acceso.usuarioId), inArray(actas.id, idsComoAsistente)));
		}
...
var load = async ({ locals, params }) => {
	...
	const [legajo, agrupamientos] = await Promise.all([obtenerLegajoAlumno(alumnoMoodleId, {
...
const imports = ["_app/immutable/nodes/39.svalTQJE.js","_app/immutable/chunks/DW4Xyvmx.js","_app/immutable/chunks/Q94VlVm2.js"];
const stylesheets = [];
const fonts = [];

export { component, fonts, imports, index, _page_server_ts as server, server_id, stylesheets };
//# sourceMappingURL=39-gteSag8D.js.map
