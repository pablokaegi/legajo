...
function _page($$renderer, $$props) {
	$$renderer.component(($$renderer) => {
		...
		for (let $$index = 0, $$length = each_array.length; $$index < $$length; $$index++) {
			...
			$$renderer.push(`<button${attr_class(`px-4 py-3 text-sm font-medium border-b-2 whitespace-nowrap transition-colors ${stringify(modo === m.id ? "border-indigo-600 text-indigo-700 bg-indigo-50" : "border-transparent text-gray-600 hover:bg-gray-50")}`)}>${escape_html(m.lbl)}</button>`);
		}
		...
		$$renderer.push(`<!--]--> `);
		$$renderer.push("<!--[-1-->");
		$$renderer.push(`<!--]--> <button class="text-xs text-indigo-600 hover:underline ml-auto">💾 Guardar esta agrupación</button></div> `);
		$$renderer.push("<!--[-1-->");
		$$renderer.push(`<!--]--> `);
		if (gruposActuales().length === 0) {
			$$renderer.push("<!--[0-->");
			$$renderer.push(`<div class="card text-center py-8"><p class="text-gray-500 text-sm">No se pudieron armar grupos.</p></div>`);
		} else {
			$$renderer.push("<!--[-1-->");
			$$renderer.push(`<div class="grid gap-2 sm:grid-cols-2"><!--[-->`);
			const each_array_2 = ensure_array_like(gruposActuales());
			for (let i = 0, $$length = each_array_2.length; i < $$length; i++) {
				let grupo = each_array_2[i];
				$$renderer.push(`<div class="card space-y-2"><div class="flex items-center justify-between"><p class="text-sm font-semibold text-gray-800">Grupo ${escape_html(i + 1)}</p> `);
				if (data.afinidad[i]) {
					$$renderer.push("<!--[0-->");
				...
				const each_array_3 = ensure_array_like(grupo);
				for (let j = 0, $$length = each_array_3.length; j < $$length; j++) {
					let m = each_array_3[j];
					$$renderer.push(`<div class="flex items-center gap-2"><div class="w-7 h-7 rounded-full bg-indigo-100 text-indigo-700 flex items-center justify-center text-xs font-bold flex-shrink-0">${escape_html(iniciales(m.nombre))}</div> <span class="text-sm text-gray-800 flex-1">${escape_html(m.nombre)}</span> `);
					$$renderer.push("<!--[-1-->");
					$$renderer.push(`<!--]--></div>`);
				}
		...
		if (data.guardados.length > 0) {
			...
			const each_array_4 = ensure_array_like(data.guardados);
...
//# sourceMappingURL=_page.svelte-CbdkUpOv.js.map
