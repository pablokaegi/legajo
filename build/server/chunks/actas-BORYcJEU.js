import { m as actas, d as db, n as actasAsistentes, p as actasAlumnos, q as actasTareas, t as actasVersiones } from './db-BwfbdrtT.js';
import { e as esDirectivoOAdmin } from './authz-CM_w6bbu.js';
...
*/
async function usuarioPuedeVerActa(usuarioId, acta) {
...
*/
async function usuarioPuedeEditarActa(usuarioId, actaId) {
	const [a] = await db.select({ createdBy: actas.createdBy }).from(actas).where(eq(actas.id, actaId)).limit(1);
	if (!a) return false;
...
async function listarActas(filtros) {
	...
	if (!filtros.verTodas && filtros.usuarioId != null) {
		const idsComoAsistente = db.select({ id: actasAsistentes.actaId }).from(actasAsistentes).where(eq(actasAsistentes.usuarioId, filtros.usuarioId));
		conditions.push(or(eq(actas.createdBy, filtros.usuarioId), inArray(actas.id, idsComoAsistente)));
	}
	return await db.select().from(actas).where(conditions.length ? and(...conditions) : void 0).orderBy(desc(actas.fecha), desc(actas.createdAt)).limit(limit).offset(offset);
}
...
export { EditarActaSchema as E, NuevaActaSchema as N, usuarioPuedeEditarActa as a, listarActasConAlumnos as b, crearActa as c, editarActa as e, listarActas as l, obtenerActaCompleta as o, usuarioPuedeVerActa as u };
//# sourceMappingURL=actas-BORYcJEU.js.map
