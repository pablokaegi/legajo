...
function _page($$renderer, $$props) {
	$$renderer.component(($$renderer) => {
		...
		let cursosFiltrados = derived(() => (data.cursos));
		...
		if (paso === 1) {
			...
			if (data.cursos.length === 0) {
				...
				$$renderer.push("<!--[-1-->");
				$$renderer.push(`<div class="relative"><span class="absolute left-3 top-1/2 -translate-y-1/2 text-gray-400 text-sm pointer-events-none">🔍</span> <input type="search"${attr("value", busquedaCurso)} placeholder="Buscar curso..." class="form-input pl-9 w-full"/></div> `);
				if (cursosFiltrados().length === 0) {
					$$renderer.push("<!--[0-->");
					$$renderer.push(`<div class="card text-center py-6"><p class="text-gray-500 text-sm">Ningún curso coincide con la búsqueda.</p></div>`);
				} else {
					$$renderer.push("<!--[-1-->");
...
//# sourceMappingURL=_page.svelte-B8fnXTwn.js.map
