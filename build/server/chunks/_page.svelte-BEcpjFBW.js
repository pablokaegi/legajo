import { R as head, U as attr, T as ensure_array_like, V as attr_class, S as escape_html, W as stringify, K as derived } from './dev-BjcCn9Qu.js';
...
function _page($$renderer, $$props) {
	$$renderer.component(($$renderer) => {
		...
		const listaFiltrada = derived(() => data.lista.filter((acta) => {
			const q = busqueda.trim().toLowerCase();
			const matchTexto = !q || acta.titulo.toLowerCase().includes(q) || acta.resumen.toLowerCase().includes(q) || acta.alumnos.some((a) => a.alumnoNombre.toLowerCase().includes(q));
...
//# sourceMappingURL=_page.svelte-BEcpjFBW.js.map
