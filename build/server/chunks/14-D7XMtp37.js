...
import { k as eliminarGrupos, n as guardarGrupos, o as obtenerSesion, h as listarVotos, p as listarGrupos, m as media, r as redondear } from './agrupamientos-Bk5OlPpq.js';
import { a as obtenerNotasMoodle, o as obtenerConducta } from './perfilAlumnos-DyXWat7C.js';
...
var load = async ({ locals, params, url }) => {
	...
	if (url.searchParams.get("rendimiento") === "1") {
		...
		const conNota = roster.map((a) => ({
			id: a.id,
			nombre: a.nombre,
			nota: notas.get(a.id) ?? null
		}));
		rendimiento = {
			grupos: serpentina([...conNota].sort((a, b) => (b.nota ?? -1) - (a.nota ?? -1)), tamano),
			sinNota: conNota.filter((a) => a.nota == null).length
		};
	}
	const conductaMap = await obtenerConducta(roster.map((a) => a.id));
	const conducta = roster.map((a) => ({
		...a,
		...conductaMap.get(a.id)
	}));
	return {
		sesion,
		roster,
...
var actions = {
	guardar: async ({ request, locals, params }) => {
		...
		try {
			...
			if (Array.isArray(parsed)) grupos = parsed.map((g) => g.map((m) => ({
				id: Number(m.id),
...
const index = 14;
let component_cache;
const component = async () => component_cache ??= (await import('./_page.svelte-fEFV-wGN.js')).default;
const server_id = "src/routes/agrupamientos/[id]/resultados/+page.server.ts";
const imports = ["_app/immutable/nodes/14.DzTBMNIz.js","_app/immutable/chunks/85pPpyzY.js","_app/immutable/chunks/C2CxDrSD.js","_app/immutable/chunks/DsqKlT4O.js","_app/immutable/chunks/DQg3U3Yx.js","_app/immutable/chunks/BaKrtv6m.js"];
const stylesheets = [];
const fonts = [];

export { component, fonts, imports, index, _page_server_ts as server, server_id, stylesheets };
//# sourceMappingURL=14-D7XMtp37.js.map
