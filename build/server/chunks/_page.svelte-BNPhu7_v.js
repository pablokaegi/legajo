...
//#region src/routes/agrupamientos/+page.svelte
function _page($$renderer, $$props) {
	$$renderer.component(($$renderer) => {
		...
		head("1kurvkk", $$renderer, ($$renderer) => {
			$$renderer.title(($$renderer) => {
				$$renderer.push(`<title>Agrupamientos — Legajo</title>`);
			});
		});
		$$renderer.push(`<div class="space-y-4"><div class="flex items-center justify-between"><h2 class="text-lg font-bold text-gray-900">Agrupamientos</h2> <a href="/agrupamientos/nueva" class="btn-primary text-sm">+ Nueva</a></div> <p class="text-xs text-gray-500">Sociograma del curso: cargá las preferencias de los alumnos y generá grupos por
    afinidad junto con un análisis psicopedagógico de la dinámica social.</p> <div class="relative"><span class="absolute left-3 top-1/2 -translate-y-1/2 text-gray-400 text-sm pointer-events-none">🔍</span> <input type="text"${attr("value", busqueda)} placeholder="Buscar por título o curso..." class="form-input pl-9 w-full text-sm"/> `);
		...
		if (data.lista.length === 0) {
			$$renderer.push("<!--[0-->");
			$$renderer.push(`<div class="card text-center py-10"><p class="text-3xl mb-2">🧩</p> <p class="text-gray-500 text-sm">No hay sesiones de agrupamiento registradas.</p> <a href="/agrupamientos/nueva" class="mt-3 inline-block text-sm text-indigo-600 hover:underline">Crear la primera</a></div>`);
...
//# sourceMappingURL=_page.svelte-BNPhu7_v.js.map
