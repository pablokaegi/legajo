import { h as head, e as escape_html, b as attr, a as ensure_array_like, c as attr_class, s as stringify, d as derived } from './dev-BsmPEhme.js';
import { p as page } from './state-BwscpIJV.js';
import './client2-BkAdULVF.js';
import './internal-GRhFhoCD.js';
...
function _page($$renderer, $$props) {
	$$renderer.component(($$renderer) => {
		...
		const esPreceptorODir = derived(() => page.data.usuario?.roles?.some((r) => ["preceptor", "directivo"].includes(r)) ?? false);
		...
		head("s0lrw2", $$renderer, ($$renderer) => {
			$$renderer.title(($$renderer) => {
				$$renderer.push(`<title>${escape_html(data.cursoNombre)} — Legajo</title>`);
			});
		});
		$$renderer.push(`<div class="space-y-4"><div><a href="/cursos" class="text-indigo-600 text-sm hover:underline">← Cursos</a> <h1 class="text-xl font-bold text-gray-900 mt-1">${escape_html(data.cursoNombre)}</h1></div> `);
		if (data.error) {
			$$renderer.push("<!--[0-->");
			...
			$$renderer.push("<!--[-1-->");
			$$renderer.push(`<div class="relative"><span class="absolute left-3 top-1/2 -translate-y-1/2 text-gray-400 text-sm pointer-events-none">🔍</span> <input type="search"${attr("value", busqueda)} placeholder="Buscar alumno..." class="form-input pl-8" autofocus=""/></div> <div class="flex items-center justify-between"><p class="text-sm text-gray-500">${escape_html(alumnosFiltrados().length)}${escape_html("")} alumno/s</p> `);
			if (esPreceptorODir()) {
				$$renderer.push("<!--[0-->");
			...
			for (let $$index = 0, $$length = each_array.length; $$index < $$length; $$index++) {
				...
				$$renderer.push(`<!--]--></div> <div class="flex gap-2 flex-shrink-0"><a${attr("href", `/cursos/${stringify(data.cursoId)}/alumnos/${stringify(alumno.id)}/notas?nombre=${stringify(encodeURIComponent(alumno.fullname))}`)} class="text-sm text-indigo-600 hover:underline" title="Ver notas">📊</a> <a${attr("href", `/observaciones/nueva?cursoId=${stringify(data.cursoId)}&alumnoId=${stringify(alumno.id)}&alumnoNombre=${stringify(encodeURIComponent(alumno.fullname))}`)} class="text-sm text-indigo-600 hover:underline" title="Registrar observación">✏️</a> `);
				if (esPreceptorODir()) {
					$$renderer.push("<!--[0-->");
					$$renderer.push(`<a${attr("href", `/preceptor/legajo/${stringify(alumno.id)}`)} class="text-sm text-indigo-600 hover:underline" title="Ver legajo completo">📋</a>`);
				} else $$renderer.push("<!--[-1-->");
...
//# sourceMappingURL=_page.svelte-B5ruxpoU.js.map
