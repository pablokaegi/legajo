...
function _page($$renderer, $$props) {
	$$renderer.component(($$renderer) => {
      ...
      _________________________________, autorizo su participación en la salida didáctica descripta arriba.</p> <div class="flex gap-8 mt-6"><div class="flex-1"><div class="border-b border-gray-400 mb-1 h-8"></div> <p class="text-xs text-center">Firma y aclaración</p></div> <div class="w-36"><div class="border-b border-gray-400 mb-1 h-8"></div> <p class="text-xs text-center">Fecha</p></div></div></div> <p class="text-xs text-gray-400 text-center">${escape_html(INSTITUCION.nombre)} · ${escape_html(INSTITUCION.direccion)}</p> <div class="no-print mt-6 text-center svelte-19nw819"><button class="btn-primary text-sm">🖨 Imprimir</button> <a${attr("href", `/institucional/salidas/${stringify(salida.id)}`)} class="ml-3 text-sm text-indigo-600 hover:underline">← Volver</a></div></div>`);
			...
			$$renderer.push(`<!--]--></div> <div class="border border-gray-200 rounded-xl overflow-hidden"><div class="bg-gray-50 px-4 py-3 flex items-center justify-between"><div class="flex items-center gap-2"><span class="text-sm font-semibold text-gray-700">👥 Autorizaciones</span> `);
			if (autorizaciones.length > 0) {
				$$renderer.push("<!--[0-->");
				$$renderer.push(`<span class="text-xs bg-green-600 text-white px-1.5 py-0.5 rounded-full">${escape_html(recibidas())}/${escape_html(autorizaciones.length)}</span>`);
			} else $$renderer.push("<!--[-1-->");
			$$renderer.push(`<!--]--></div> <button type="button" class="text-xs text-indigo-600 hover:underline font-medium">${escape_html("+ Agregar alumnos")}</button></div> `);
			if (autorizaciones.length === 0) {
				$$renderer.push("<!--[0-->");
				$$renderer.push(`<div class="px-4 py-5 text-center text-sm text-gray-400">No hay alumnos agregados todavía.<br/> <span class="text-xs">Usá "+ Agregar alumnos" para seleccionar desde Moodle.</span></div>`);
			} else {
				$$renderer.push("<!--[-1-->");
				if (pendientes() > 0) {
					$$renderer.push("<!--[0-->");
				...
				for (let $$index = 0, $$length = each_array.length; $$index < $$length; $$index++) {
					...
					$$renderer.push(`<div class="px-4 py-3 flex items-center gap-3"><div${attr_class(`w-8 h-8 rounded-full flex-shrink-0 flex items-center justify-center text-xs font-bold ${stringify(aut.documentoPath ? "bg-green-100 text-green-700" : "bg-gray-100 text-gray-500")}`)}>${escape_html(aut.alumnoNombre.split(" ").map((w) => w[0]).slice(0, 2).join(""))}</div> <div class="flex-1 min-w-0"><p class="text-sm font-medium text-gray-900 truncate">${escape_html(aut.alumnoNombre)}</p> `);
					if (aut.documentoPath) {
						$$renderer.push("<!--[0-->");
						$$renderer.push(`<p class="text-xs text-green-600">✅ Subido ${escape_html(aut.documentoSubidoAt ? new Date(aut.documentoSubidoAt).toLocaleDateString("es-AR") : "")}</p>`);
					} else {
						$$renderer.push("<!--[-1-->");
						$$renderer.push(`<p class="text-xs text-amber-600">⏳ Pendiente</p>`);
					}
					$$renderer.push(`<!--]--></div> <div class="flex gap-2 flex-shrink-0 items-center">`);
					if (aut.documentoPath) {
						$$renderer.push("<!--[0-->");
						$$renderer.push(`<a${attr("href", `/autorizar/${stringify(aut.uploadToken)}/archivo`)} target="_blank" class="text-xs text-indigo-600 hover:underline" title="Ver autorización">Ver</a>`);
					} else $$renderer.push("<!--[-1-->");
					$$renderer.push(`<!--]--> <button type="button" class="text-xs text-gray-400 hover:text-gray-700" title="Copiar link">🔗</button></div></div>`);
				}
				$$renderer.push(`<!--]--></div>`);
			}
			$$renderer.push(`<!--]--> `);
...
//# sourceMappingURL=_page.svelte-Bvx2WJ3I.js.map
