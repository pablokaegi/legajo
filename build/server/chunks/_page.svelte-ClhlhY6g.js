import { R as head, S as escape_html, T as ensure_array_like, V as attr_class, a9 as clsx$1 } from './dev-BjcCn9Qu.js';
...
function _page($$renderer, $$props) {
	$$renderer.component(($$renderer) => {
		...
		$$renderer.push(`<!--]--></a> <a href="/observaciones/historial" class="card flex flex-col items-center py-5 gap-2 hover:border-indigo-300 transition-colors text-center"><span class="text-3xl">📋</span> <span class="font-medium text-gray-800 text-sm">Historial</span></a> <a href="/admin/status" class="card flex flex-col items-center py-5 gap-2 hover:border-indigo-300 transition-colors text-center"><span class="text-3xl">🔧</span> <span class="font-medium text-gray-800 text-sm">Estado Moodle</span></a></div> `);
		if (data.logs.length > 0) {
			$$renderer.push("<!--[0-->");
			$$renderer.push(`<div><h2 class="font-semibold text-gray-700 text-sm mb-2">Últimas actividades</h2> <div class="space-y-1.5"><!--[-->`);
			const each_array = ensure_array_like(data.logs);
			for (let $$index = 0, $$length = each_array.length; $$index < $$length; $$index++) {
				let log = each_array[$$index];
				$$renderer.push(`<div class="flex items-start gap-2 text-xs text-gray-500 bg-white rounded-lg px-3 py-2 border border-gray-100"><span${attr_class(clsx$1(log.status === "ok" ? "text-green-500" : "text-red-500"))}>●</span> <div class="flex-1 min-w-0"><span class="font-medium text-gray-700">${escape_html(log.tipo)}</span> <span class="mx-1">—</span> <span class="truncate">${escape_html(log.mensaje)}</span></div> <span class="text-gray-300 whitespace-nowrap">${escape_html(new Date(log.createdAt).toLocaleTimeString("es-AR", {
					hour: "2-digit",
					minute: "2-digit"
				}))}</span></div>`);
			}
...
//# sourceMappingURL=_page.svelte-ClhlhY6g.js.map
